...
| `--mode` | `readable` \| `random` \| `word` \| `translit` \| `dictionary` \| `compound` \| `both` | Слоговые, случайные, с обязательным словом, русские слова транслитом, настоящие английские словарные слова, составные из двух токенов, или оба обычных генератора |
...
| `--safe-mode` | флаг | Случайная пауза из широкого диапазона вместо фиксированной ±30%; поднимает минимум до 2000 мс, если `--delay` меньше |
...
# Одно русское существительное транслитом: mechta, volna, zoloto и другие
npm run search -- --mode translit --min-length 5 --max-length 12 --digits exclude --count 30

# Настоящие английские словарные слова целиком: acorn, glory, sector...
npm run search -- --mode dictionary --min-length 5 --max-length 8 --digits exclude --count 30

# Составные из двух токенов: goldshop, topauto, probank...
npm run search -- --mode compound --min-length 6 --max-length 10 --digits exclude --count 30
```
...
Каждый кандидат — точный транслит **одного существительного** (`mechta`, `volna`, `zoloto`). Генератор не соединяет несколько слов, не добавляет `_`, цифры или произвольные суффиксы и не обрезает слова посередине. Поэтому после исчерпания подходящих слов он может вернуть меньше запрошенного `--count`.

`--digits require` несовместим с этим режимом и отклоняется с понятным сообщением. Значения `exclude` и `allow` сохраняют существительное без изменений.

### Словарные и составные режимы

`--mode dictionary` и `--mode compound` появились из наблюдения над собственными данными проекта: в `priceModel/features.ts` `isDictionaryWord`/`containsDictionaryWord` и `hasPopularToken` — одни из главных факторов цены (реальные топовые продажи Fragment из `data/sold-history.json` — это `news`, `auto`, `bank`, `avia` и подобные целые слова), но ни `readable`, ни `random`, ни `word` не производили такие имена намеренно.

- **`dictionary`** — кандидат сам является настоящим английским словом из локального частотного словаря (`priceModel/dictionaryWords.ts`, ~8.7k слов, google-10000-english, отфильтрованный до 3–10 символов). `--digits require` добавляет цифровой суффикс в конец, не трогая само слово; если места не осталось — слово пропускается, а не обрезается.
- **`compound`** — два коротких токена (словарное слово и/или курируемый бренд-токен из `brandTokens.ts`: `gold`, `top`, `pay`, `shop`, `vip`...) склеиваются в одно имя (`goldshop`, `topauto`, `probank`). Половина попыток намеренно комбинирует курируемый токен со словарным словом — это надёжнее читается как осмысленный бренд, чем случайная пара из всего словаря. Словарные части дополнительно фильтруются простой проверкой на произносимость (есть гласная, нет разбега согласных длиннее трёх подряд) — исходный частотный список полон аббревиатур вроде `tmp`/`cms`/`jvc`, которые в противном случае лезли бы в компаунды.

Оба режима поддерживают все три политики цифр и обе стороны (`telegram`/`fragment`) как обычно.

Это не идеальная лингвистическая курация — источник слов остаётся частотным списком, а не размеченным словарём частей речи, так что иногда попадаются странные компаунды (имена собственные, редкие термины). Для более чистого результата можно сузить `priceModel/dictionaryWords.ts` вручную под свою нишу.
...
```bash
npm run train-generator -- --epochs 100 --dictionary-words 1200
```

Посимвольная модель обучается на двух источниках: `data/sold-history.json` и ограниченной выборке `priceModel/dictionaryWords.ts`. Контекст из четырёх символов используется для предсказания следующего. Избранное (`favorites.json`) в обучение намеренно не попадает — это ручной шорт-лист конкретных имён «на посмотреть/купить», а не датасет: на типичном небольшом личном списке модель быстро переобучилась бы именно под него, а не под общие рыночные паттерны.

Продажи участвуют в обучении не поровну — вес записи зависит от перцентиля её цены среди остальных собранных продаж: нижний квартиль (почти всегда шум по цене-полу рынка вроде случайных 5-буквенных строк за 10 TON) отбрасывается совсем, а верхний квартиль повторяется в четыре раза чаще. Без этого взвешивания сеть училась поровну и на `auto` (900 000 TON), и на рыночном мусоре — а его в сырой истории продаж примерно столько же, сколько по-настоящему ценных имён.

`--dictionary-words <n>` (по умолчанию 1200, `0` отключает) добавляет случайную выборку настоящих английских слов как образец правильной фонотактики — не для запоминания конкретных слов (для этого есть `--mode dictionary`/`--mode compound` в поиске), а чтобы модель лучше улавливала, как вообще звучит настоящее слово, а не только то, что случайно попало в собранные продажи. Полный словарь (~8.7k слов) увеличил бы время обучения в разы; ограниченная выборка — разумный компромисс.

Чем разнообразнее корпус, тем полезнее результат.
